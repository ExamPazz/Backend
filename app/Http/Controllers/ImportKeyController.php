    ...
    public function importStructureforMaths(Request $request)
{
    ...
    try {
        ...
        foreach ($sheet->getRowIterator(2) as $row) {
            ...
            $sectionRaw = trim($cells->current()->getValue() ?? ''); $cells->next();
            $chapterRaw = trim($cells->current()->getValue() ?? ''); $cells->next();
            $topicRaw = trim($cells->current()->getValue() ?? ''); $cells->next();
            $objectiveRaw = trim($cells->current()->getValue() ?? '');
            ...
            // Handle Chapter
            if (!empty($chapterRaw) && $currentSection) {
                [$chapterCode, $chapterBody] = array_pad(explode('.', $chapterRaw, 2), 2, null);
            ...
            // Handle Topic
            if (!empty($topicRaw) && $currentChapter) {
                $topicLines = explode("\n", $topicRaw);

                foreach ($topicLines as $topicLine) {
                    $topicLine = trim($topicLine);
                    if (empty($topicLine)) continue;

                    [$topicCode, $topicBody] = array_pad(preg_split('/[.:]/', $topicLine, 2), 2, null);

                    $currentTopic = Topic::firstOrCreate([
                        'subject_id' => $subject->id,
            ...
            // Handle Objective
            if (!empty($objectiveRaw) && $currentTopic) {
                $objectiveLines = preg_split('/[;\n]/', $objectiveRaw);

                foreach ($objectiveLines as $line) {
                    $line = trim(str_replace("\u{00A0}", '', $line));
                    if (empty($line)) continue;
